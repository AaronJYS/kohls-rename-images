...
function worksheetXML(records, columns) {
  ...
  const rows = records.map((row, index) => {
    const listLines = Math.max(...["sku", "upc"].map((key) => String(row[key] ?? "").split("\n").length));
    const height = Math.min(409, Math.max(18, listLines * 15 + 4));
    return `<row r="${index + 2}" ht="${height}" customHeight="1">${columns.map(([key, , type], col) => {
      ...
      return textCell(ref, value, key === "sku" || key === "upc" ? 7 : 1);
    }).join("")}</row>`;
  }).join("");
  return `${XML}<worksheet xmlns="${NS}"><dimension ref="A1:${last}"/><sheetViews><sheetView showGridLines="0" workbookViewId="0"><pane ySplit="1" topLeftCell="A2" activePane="bottomLeft" state="frozen"/><selection pane="bottomLeft" activeCell="A2" sqref="A2"/></sheetView></sheetViews><sheetFormatPr defaultRowHeight="18"/><cols>${widths.map((width, i) => `<col min="${i + 1}" max="${i + 1}" width="${width}" customWidth="1"/>`).join("")}</cols><sheetData>${header}${rows}</sheetData><autoFilter ref="A1:${last}"/></worksheet>`;
}

export function workbookParts(records) {
  if (!records.length) throw new Error("There are no extracted purchase orders to export.");
  if (records.reduce((count, row) => count + (row.line_count ?? 1), 0) > 100000)
    throw new Error("Export is limited to 100,000 source line items per PDF.");
  const overflow = records.filter((row) => [row.sku, row.upc].some(exceedsExcelCellLimits));
  const overflowSet = new Set(overflow);
  const orderRows = records.map((row) => overflowSet.has(row)
    ? { ...row, sku: `${row.line_count} values (see Order Items)`, upc: `${row.line_count} values (see Order Items)` }
    : row);
  const sheets = [{ name: "Purchase Orders", rows: orderRows, columns: COLUMNS }];
  if (overflow.length) {
    if (overflow.some((row) => !row.line_items?.length)) throw new Error("Complete SKU/UPC pairs are required to export long identifier lists.");
    sheets.push({ name: "Order Items", rows: overflow.flatMap((row) => row.line_items), columns: [
      ["po", "Document Num", "text"], ["line_no", "Line Num", "text"],
      ["sku", "User Defined Field #1", "text"], ["upc", "User Defined Field #2", "text"],
    ] });
  }
  const parts = {
    ...
    "xl/styles.xml": `${XML}<styleSheet xmlns="${NS}"><numFmts count="2"><numFmt numFmtId="164" formatCode="m/d/yyyy"/><numFmt numFmtId="165" formatCode="mm/dd/yyyy"/></numFmts><fonts count="2"><font><sz val="11"/><name val="Arial"/></font><font><b/><sz val="11"/><color rgb="FFFFFFFF"/><name val="Arial"/></font></fonts><fills count="3"><fill><patternFill patternType="none"/></fill><fill><patternFill patternType="gray125"/></fill><fill><patternFill patternType="solid"><fgColor rgb="FF1F4E78"/><bgColor indexed="64"/></patternFill></fill></fills><borders count="2"><border><left/><right/><top/><bottom/><diagonal/></border><border><left/><right/><top/><bottom style="thin"><color rgb="FFD9D9D9"/></bottom><diagonal/></border></borders><cellStyleXfs count="1"><xf numFmtId="0" fontId="0" fillId="0" borderId="0"/></cellStyleXfs><cellXfs count="8"><xf numFmtId="0" fontId="0" fillId="0" borderId="0" xfId="0"/><xf numFmtId="49" fontId="0" fillId="0" borderId="1" xfId="0" applyNumberFormat="1" applyAlignment="1"><alignment horizontal="left" vertical="top"/></xf><xf numFmtId="3" fontId="0" fillId="0" borderId="1" xfId="0" applyNumberFormat="1" applyAlignment="1"><alignment horizontal="right" vertical="top"/></xf><xf numFmtId="4" fontId="0" fillId="0" borderId="1" xfId="0" applyNumberFormat="1" applyAlignment="1"><alignment horizontal="right" vertical="top"/></xf><xf numFmtId="0" fontId="1" fillId="2" borderId="0" xfId="0" applyAlignment="1"><alignment horizontal="center" vertical="center"/></xf><xf numFmtId="164" fontId="0" fillId="0" borderId="1" xfId="0" applyNumberFormat="1" applyAlignment="1"><alignment horizontal="center" vertical="top"/></xf><xf numFmtId="165" fontId="0" fillId="0" borderId="1" xfId="0" applyNumberFormat="1" applyAlignment="1"><alignment horizontal="center" vertical="top"/></xf><xf numFmtId="49" fontId="0" fillId="0" borderId="1" xfId="0" applyNumberFormat="1" applyAlignment="1"><alignment horizontal="left" vertical="top" wrapText="1"/></xf></cellXfs><cellStyles count="1"><cellStyle name="Normal" xfId="0" builtinId="0"/></cellStyles></styleSheet>`,
  };
